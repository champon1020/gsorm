...
func TestSelectStmt_BuildQuerySQL_Fail(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			func() error {
				// Prepare for test.
				s := gsorm.Select(nil, "").(*statement.SelectStmt)
...
func TestSelectStmt_CompareStmts_Fail(t *testing.T) {
	testCases := []struct {
		...
		{
			gsorm.Select(nil, "column1").From("table").(*statement.SelectStmt),
			gsorm.Select(nil, "column1").From("table").Where("column1 = ?", 10).(*statement.SelectStmt),
			statement.ErrInvalidValue,
		},
		{
			gsorm.Select(nil, "column1").From("table").Where("column1 = ?", 10).(*statement.SelectStmt),
			gsorm.Select(nil, "column1").From("table").Where("column1 = ?", 100).(*statement.SelectStmt),
			statement.ErrInvalidValue,
		},
	}
	...
	testCases := []struct {
		...
		},
		{
			gsorm.Select(nil).
				...
				Having("SUM(id) = ?", 10).
				RawClause("RAW").
				Union(gsorm.Select(nil).From("table2")).(*statement.SelectStmt),
			`SELECT * FROM table ` +
				`HAVING SUM(id) = 10 ` +
		...
		},
		{
			gsorm.Select(nil).
				...
				Having("SUM(id) = ?", 10).
				RawClause("RAW").
				UnionAll(gsorm.Select(nil).From("table2")).(*statement.SelectStmt),
			`SELECT * FROM table ` +
				`HAVING SUM(id) = 10 ` +
...
func TestSelectStmt_From(t *testing.T) {
	testCases := []struct {
		...
		{
			gsorm.Select(nil, "emp_no").From("employees").(*statement.SelectStmt),
			`SELECT emp_no FROM employees`,
		},
		{
			gsorm.Select(nil, "emp_no").From("employees AS e").(*statement.SelectStmt),
		...
		{
			gsorm.Select(nil, "emp_no", "dept_no").From("employees", "departments").(*statement.SelectStmt),
...
func TestSelectStmt_Where(t *testing.T) {
	testCases := []struct {
		...
		{
			gsorm.Select(nil).From("employees").
				Where("emp_no IN (?)", gsorm.Select(nil, "emp_no").From("dept_manager")).(*statement.SelectStmt),
...
func TestSelectStmt_And(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			gsorm.Select(nil).From("employees").
				Where("emp_no = ?", 1001).
				And("emp_no IN (?)", gsorm.Select(nil, "emp_no").From("dept_manager")).(*statement.SelectStmt),
...
func TestSelectStmt_Or(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			gsorm.Select(nil).From("employees").
				Where("emp_no = ?", 1001).
				Or("emp_no IN (?)", gsorm.Select(nil, "emp_no").From("dept_manager")).(*statement.SelectStmt),
...
func TestSelectStmt_Having(t *testing.T) {
	testCases := []struct {
		...
		{
			gsorm.Select(nil, "emp_no", "AVG(salary)").From("salaries").
				Having("emp_no IN (?)", gsorm.Select(nil, "emp_no").From("dept_manager")).(*statement.SelectStmt),
			`SELECT emp_no, AVG(salary) FROM salaries ` +
	...
	testCases := []struct {
	...
	}{
		{
			gsorm.Select(nil, "emp_no", "dept_no").From("dept_manager").
				Union(gsorm.Select(nil, "emp_no", "dept_no").From("dept_emp")).(*statement.SelectStmt),
...
func TestSelectStmt_UnionAll(t *testing.T) {
	testCases := []struct {
		...
		{
			gsorm.Select(nil, "emp_no", "dept_no").From("dept_manager").
				UnionAll(gsorm.Select(nil, "emp_no", "dept_no").From("dept_emp")).(*statement.SelectStmt),
			`SELECT emp_no, dept_no FROM dept_manager ` +
...
func TestSelectStmt_Limit(t *testing.T) {
	testCases := []struct {
		...
		{
			gsorm.Select(nil).From("employees").Limit(10).(*statement.SelectStmt),
			`SELECT * FROM employees LIMIT 10`,
		},
		{
			gsorm.Select(nil).From("employees").Limit(10).Offset(5).(*statement.SelectStmt),
