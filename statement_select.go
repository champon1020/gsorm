...
func (s *SelectStmt) From(tables ...string) ifc.FromMP {
	f := new(clause.From)
...
func (s *SelectStmt) Where(expr string, vals ...interface{}) ifc.WhereMP {
	s.call(&clause.Where{Expr: expr, Values: vals})
	return s
}

// And calls AND clause.
func (s *SelectStmt) And(expr string, vals ...interface{}) ifc.AndMP {
	s.call(&clause.And{Expr: expr, Values: vals})
	return s
}

// Or calls OR clause.
func (s *SelectStmt) Or(expr string, vals ...interface{}) ifc.OrMP {
	s.call(&clause.Or{Expr: expr, Values: vals})
	return s
}

// Limit calls LIMIT clause.
func (s *SelectStmt) Limit(num int) ifc.LimitMP {
	s.call(&clause.Limit{Num: num})
	return s
}

// Offset calls OFFSET clause.
func (s *SelectStmt) Offset(num int) ifc.OffsetMP {
	s.call(&clause.Offset{Num: num})
	return s
}

// OrderBy calls ORDER BY clause.
func (s *SelectStmt) OrderBy(cols ...string) ifc.OrderByMP {
	s.call(&clause.OrderBy{Columns: cols})
	return s
}

// Join calls (INNER) JOIN clause.
func (s *SelectStmt) Join(table string) ifc.JoinMP {
...
func (s *SelectStmt) LeftJoin(table string) ifc.JoinMP {
...
func (s *SelectStmt) RightJoin(table string) ifc.JoinMP {
...
func (s *SelectStmt) FullJoin(table string) ifc.JoinMP {
...
// On calls ON clause.
func (s *SelectStmt) On(expr string, vals ...interface{}) ifc.OnMP {
	s.call(&clause.On{Expr: expr, Values: vals})
	return s
}

// Union calls UNION clause.
func (s *SelectStmt) Union(stmt syntax.Stmt) ifc.UnionMP {
	s.call(&clause.Union{Stmt: stmt, All: false})
	return s
}

// UnionAll calls UNION ALL clause.
func (s *SelectStmt) UnionAll(stmt syntax.Stmt) ifc.UnionMP {
	s.call(&clause.Union{Stmt: stmt, All: true})
	return s
}

// GroupBy calls GROUP BY clause.
func (s *SelectStmt) GroupBy(cols ...string) ifc.GroupByMP {
...
func (s *SelectStmt) Having(expr string, vals ...interface{}) ifc.HavingMP {
